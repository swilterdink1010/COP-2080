...
with st.sidebar.expander("➕ Add New Task", expanded=True):
    ...
    if st.button("Add Task"):
        if new_title.strip():
            payload = {"title": new_title.strip(), "desc": new_desc.strip()}
...
if tasks:
    ...
    with st.sidebar.expander("✏️ Update Task"):
        ...
        upd_desc  = st.text_area("New description",  value=selected_task["desc"])
        upd_done  = st.checkbox("Completed",         value=selected_task["complete"])
...
with col1:
    ...
    if tasks:
        df = pd.DataFrame(tasks)[["id","title","desc","complete","created_at"]]
        df["completed"] = df["complete"].map({True: "✅ Done", False: "⏳ Pending"})
